...
type API struct {
	...
	subFile          string                 // file name will be clientid.json under subscribers/ folder
...
func NewSubscriber(clientID string, healthEndpoint *types.URI) subscriber.Subscriber {
...
func (p *API) CreateSubscription(clientID string, healthLink *types.URI, sub pubsub.PubSub) (subscriptionClient *subscriber.Subscriber, err error) {
	var ok bool
	if subscriptionClient, ok = p.HasClient(clientID); ok {
		if subExists, ok := p.HasSubscription(clientID, sub.GetResource()); ok {
			log.Warnf("there was already a subscriptionOne in the store,will override existing subscriptionOne %v", subExists)
		}
	}

	if subscriptionClient == nil {
		subscriptionClient = subscriber.New(clientID, healthLink)
	}
	if sub.ID == "" { //this will be always set by rest api
		log.Warnf("there is missing subsription ID ,skipping creation %v", sub)
		err = errors.New(fmt.Sprintf("there is missing subsription ID ,skipping creation %v", sub))
		return
	}

	subscriptionClient.SubStore.Set(sub.ID, sub)
...
func writeToFile(subscriberClient subscriber.Subscriber, filePath string) error {
	...
	if len(b) > 0 {
		...
		persistedSubClient = *subscriber.New(subscriberClient.ClientID, subscriberClient.HealthEndPoint)
	} // no  file found
