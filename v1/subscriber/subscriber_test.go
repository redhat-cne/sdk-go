...
var (
	...
	subscriberWithOneEventCheck = subscriber.Subscriber{
		...
		HealthEndPoint: &types.URI{URL: url.URL{Scheme: "http", Host: "localhost:8080", Path: "/helathz"}},
		Status:         0,
	}
...
func TestAPI_CreateSubscription(t *testing.T) {
	defer clean()
	s, e := globalInstance.CreateSubscription(clientID, clientHealthEndPoint, *subscriptionOne)
	assert.Nil(t, e)
...
func TestAPI_CreateTwoSubscription(t *testing.T) {
	...
	assert.NotNil(t, s.SubStore.Store)
	assert.Equal(t, 1, len(s.SubStore.Store))
	s, e = globalInstance.CreateSubscription(clientID, clientHealthEndPoint, *subscriptionTwo)
	assert.Nil(t, e)
	assert.NotEmpty(t, s.ClientID)
	assert.NotNil(t, s.SubStore.Store)
